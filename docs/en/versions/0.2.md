...
The CLI and LSP share one project semantic model. Diagnostics, completion, hover, definition, references, and rename work across the loaded project while enforcing module exports and private visibility. `module.norm` is one `Module(name, version, exports)` object expression. For module `std`, export `math.integer` identifies `std/math/integer.norm`.
