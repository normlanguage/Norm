...
1. **Java 编写全部核心工具链。** Lexer、Parser、AST、名称解析、类型检查、Typed IR、格式化器、LSP 共享组件、包工具核心逻辑和 CLI 均以 Java 实现。
2. **Truffle/GraalVM 是唯一官方执行后端。** Norm 程序通过 Truffle language implementation 在 GraalVM 上执行。Typed IR 不直接面向 LLVM、Cranelift 或自研机器码后端。
