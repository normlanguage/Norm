...
`app/sample/bbs` 是纯 Norm Micronaut 应用。`application.norm` 使用 `MicronautConfig` 下的 `micronaut`、`datasources`、`jpa` 与 `endpoints` 类型树声明应用配置，结构直接对应 `application.yml`。`std.configuration` 从这棵树派生 Micronaut 属性，启动器不维护字符串 key。`module.norm` 只声明三个生产依赖。应用按 `Web.norm`、`service`、`repository` 三层组织；Repository 通过 Norm ORM 的类型化查询访问 Hibernate，Service 持有事务和业务对象创建，Web 层只处理 HTTP。Hibernate Provider 和官方 Micronaut Processor 接入持久化上下文、Controller、构造器 DI、Serde、Validation、Security 和 Filter。
