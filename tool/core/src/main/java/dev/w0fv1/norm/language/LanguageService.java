...
public final class LanguageService {
  ...
  public List<Completion> complete(AnalysisResult analysis, int offset) {
    ...
    if (offset > 0 && source.text().charAt(offset - 1) == '.') {
      return memberCompletions(analysis.semanticModel(), source.text(), offset - 1);
    }
    List<Completion> result = new ArrayList<>();
    LanguageSyntax.completionKeywords()
        .forEach(
            keyword ->
                result.add(
                    new Completion(
  ...
  public Optional<SourceLocation> definition(AnalysisResult analysis, int offset) {
    return analysis.semanticModel().symbolAt(offset).flatMap(Symbol::declaration);
  }
  ...
  public Optional<RenameTarget> prepareRename(AnalysisResult analysis, int offset) {
    SemanticModel model = analysis.semanticModel();
    Optional<Symbol> symbol = model.symbolAt(offset);
    if (symbol.isEmpty() || symbol.orElseThrow().declaration().isEmpty()) return Optional.empty();
    return model
        .referenceAt(offset)
  ...
  public Optional<RenameEdit> rename(AnalysisResult analysis, int offset, String newName) {
    ...
    SemanticModel model = analysis.semanticModel();
    Optional<Symbol> selected = model.symbolAt(offset);
    if (selected.isEmpty() || selected.orElseThrow().declaration().isEmpty())
  ...
  private static List<Completion> memberCompletions(
    ...
    if (start == dotOffset) return List.of();
    int receiverOffset = start;
    Optional<SemanticType> receiverType =
        model.symbolAt(receiverOffset).map(Symbol::type).or(() -> model.typeAt(receiverOffset));
    ...
    if (symbol.kind() == SymbolKind.FUNCTION || symbol.kind() == SymbolKind.METHOD) {
      String parameters =
          symbol.parameters().stream()
              ...
              .collect(java.util.stream.Collectors.joining(", "));
      return symbol.type().displayName() + " " + symbol.name() + "(" + parameters + ")";
    }
    if (symbol.kind() == SymbolKind.TYPE) return symbol.name();
