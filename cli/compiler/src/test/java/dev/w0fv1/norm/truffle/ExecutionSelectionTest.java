...
final class ExecutionSelectionTest {
            ...
            .filter(
                candidate ->
                    candidate.definition()
                            instanceof dev.w0fv1.norm.core.CoreDefinition.Aggregate aggregate
                        && aggregate.nominalType().name().equals("Item"))
            .findFirst()
            .orElseThrow();
    var aggregate = (dev.w0fv1.norm.core.CoreDefinition.Aggregate) record.definition();
    assertFalse(aggregate.constructors().isEmpty());
    for (var constructor : aggregate.constructors()) {
      var id =
          artifact
              .program()
              .resolve(record.id(), (dev.w0fv1.norm.core.DefinitionReference) constructor);
  ...
  void onlyPreparesRequestedMethodSlots() {
    ...
    var owner =
        dev.w0fv1.norm.core.CoreTypes.absolute(
  ...
  void doesNotPrepareUnreferencedFunctionBodies() {
    ...
    assertEquals("ready" + System.lineSeparator(), output.toString());
    var external =
        dev.w0fv1.norm.core.CoreExecutionPlan.forArtifact(artifact, java.util.Set.of(dormant));
